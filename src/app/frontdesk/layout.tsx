...
export default function FrontdeskLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const allowedRoles = ["front_desk", "admin", "organiser"];
  ...
  const renderNavLink = (link: (typeof adminNavLinks)[0], opts?: { onNavigate?: () => void }) => {
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center h-14 sm:h-16 gap-2 sm:gap-6">
            ...
            <button
              ...
              aria-controls="mobile-frontdesk-nav"
            ...
            <Link href={userRole === "admin" ? "/dashboard" : userRole === "organiser" ? "/organiser-dashboard" : "/frontdesk"} className="flex items-center gap-2 sm:gap-2.5 shrink-0 min-w-0 max-w-[min(100%,22rem)]">
              ...
              <div className="flex flex-col leading-tight min-w-0">
                ...
                <span className="text-[9px] sm:text-[10px] text-accent font-bold uppercase tracking-wider sm:tracking-widest whitespace-normal dark:text-amber-400/90">
                  {userRole === "admin" ? "Admin Portal" : userRole === "organiser" ? "Organiser Portal" : "Front Desk"}
                </span>
              </div>
            </Link>
              ...
              <div className="relative shrink-0" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--card-bg)] rounded-2xl shadow-xl border border-[var(--border-subtle)] overflow-hidden animate-in fade-in zoom-in-95 duration-150">
                    ...
                    <div className="p-1.5">
                      <Link
                        href={userRole === "admin" ? "/dashboard/settings" : userRole === "organiser" ? "/organiser-dashboard/settings" : "/frontdesk/settings"}

    ...
    <div className="space-y-4 sm:space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-start">
        <div className="min-w-0">
          ...
          <p className="text-gray-500 mt-0.5 text-xs sm:text-sm font-medium">Your ticket sales at a glance</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-row gap-2 w-full sm:w-auto">
           <button
             ...
             className="inline-flex items-center justify-center min-h-[44px] px-4 py-2.5 rounded-xl bg-white border border-pink-100 hover:bg-pink-50 text-gray-800 font-bold text-xs sm:text-sm shadow-sm transition-all"
           >
      ...
      {loading ? (
        <div className="flex min-h-[12rem] items-center justify-center rounded-xl border border-pink-50 bg-white">
        ...
        <>
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-pink-100/80 overflow-hidden relative">
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
                <div className="p-4 sm:p-5 relative">
                   <div className="flex items-center gap-2 mb-1">
                      ...
                      <h3 className="text-sm font-bold text-gray-900">Sales progress</h3>
                   </div>
                   <p className="text-[11px] sm:text-xs text-gray-500 font-medium mb-3 line-clamp-2">Tickets sold vs your target</p>
                   
                   <div className="flex items-end gap-2 mb-2">
                      <span className="text-2xl sm:text-4xl font-bold text-primary tabular-nums">{overall.sold}</span>
                      <span className="text-lg sm:text-2xl font-bold text-gray-400 mb-0.5 tabular-nums">/ {overall.target}</span>
                   </div>
                ...
                <div className="p-4 sm:p-5 bg-gray-50/50 relative">
                   <div className="flex items-center gap-2 mb-1">
                      <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 shrink-0" />
                      <h3 className="text-sm font-bold text-gray-900">My revenue</h3>
                   </div>
                   <p className="text-[11px] sm:text-xs text-gray-500 font-medium mb-3">From your sales</p>
                   
                   <div className="flex items-baseline gap-1 mb-3">
                      <span className="text-base font-bold text-gray-400">₹</span>
                      <span className="text-2xl sm:text-4xl font-bold text-gray-900 tabular-nums leading-none">{new Intl.NumberFormat('en-IN').format(overall.revenue)}</span>
                   </div>
                ...
                <div className="p-4 sm:p-5 flex flex-row sm:flex-col items-center justify-between sm:justify-center gap-3 sm:text-center lg:min-w-[140px] bg-white">
                   <Calendar className="w-5 h-5 sm:w-6 sm:h-6 text-secondary shrink-0 sm:mx-auto" />
                   <div>
                      <div className="text-2xl sm:text-3xl font-bold text-gray-900 tabular-nums">15</div>
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wide">Days to event</p>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            {ticketData.map(item => {
              ...
              return (
                 <div key={item.name} className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-5 shadow-sm border border-pink-50/80 hover:border-primary/30 transition-colors">
                    <div className="flex justify-between items-start gap-1 mb-2">
                       <h3 className="text-[11px] sm:text-sm font-bold text-gray-900 leading-tight line-clamp-2">{item.name}</h3>
                    ...
                    <p className="text-[10px] sm:text-xs font-medium text-gray-500 line-clamp-2">{remain > 0 ? `${remain} to target` : 'Target met'}</p>
                 </div>
              );
            })}
          </div>

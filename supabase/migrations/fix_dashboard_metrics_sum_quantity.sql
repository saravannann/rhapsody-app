-- Update the dashboard metrics function to count HEADS (SUM(quantity)) instead of Bookings (COUNT(*))
...
BEGIN
  ...
  -- 2. Calculate Metrics (The fix: Using SUM instead of COUNT)
  ...
  SELECT json_build_object(
    'total_revenue', COALESCE(SUM(price * quantity), 0),
    'trust_revenue', COALESCE(SUM(CASE WHEN funds_destination = 'trust' THEN price * quantity ELSE 0 END), 0),
    'organizer_revenue', COALESCE(SUM(CASE WHEN funds_destination = 'organizer' THEN price * quantity ELSE 0 END), 0),
    'total_tickets', COUNT(*), -- Total Bookings
    'scannable_tickets', COALESCE(SUM(CASE WHEN status != 'cancelled' AND type NOT ILIKE '%donor%' THEN quantity ELSE 0 END), 0), -- Total Heads (Corrected)
    'checked_in', COALESCE(SUM(checked_in_count), 0), -- Total Heads Checked In (Corrected)
  ...
  FROM (
    SELECT 
      sold_by as name,
      COALESCE(SUM(CASE WHEN type = 'Platinum' THEN quantity ELSE 0 END), 0) as platinum,
      COALESCE(SUM(CASE WHEN type = 'Donor' THEN quantity ELSE 0 END), 0) as donor,
      COALESCE(SUM(CASE WHEN type = 'Student' THEN quantity ELSE 0 END), 0) as student,

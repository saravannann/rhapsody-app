-- Update the dashboard metrics function to be consistent with head counts and handle cancelled status correctly.
...
BEGIN
  ...
  SELECT json_build_object(
    -- Revenue should only count non-cancelled tickets
  ...
  -- 3. Leaderboard Data (Should only count non-cancelled tickets)
  WITH filtered_tickets AS (
    SELECT * FROM tickets
    WHERE created_at >= v_start_date
      AND (p_type_filter = 'All Types' OR type = p_type_filter)
  ...
  -- 4. Chart Data (By Category, non-cancelled only)
  WITH filtered_tickets AS (
    SELECT * FROM tickets
    WHERE created_at >= v_start_date
      AND (p_type_filter = 'All Types' OR type = p_type_filter)
  ...
  -- 5. WhatsApp Stats (Based on all entries for the filter)
  WITH filtered_tickets AS (
    SELECT * FROM tickets
    WHERE created_at >= v_start_date
      AND (p_type_filter = 'All Types' OR type = p_type_filter)
